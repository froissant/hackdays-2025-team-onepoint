...
import { ProjectFileStorageService } from "../services/ProjectFileStorageService";
...
const TLDrawSyncController: FastifyPluginCallback = (app, _, done) => {
	...
	const projectStorageService = new ProjectFileStorageService(app.log);
	...
	app.get('/connect/:roomId', { websocket: true }, async (socket, req : FastifyRequest<{ Params: { roomId: string }, Querystring: { sessionId: string } }>) => {
	...
	app.put('/uploads/:id', {}, async (req: FastifyRequest<{ Params: { id: string } }>, res) => {
		const id = req.params.id;
		await storageService.saveAsset(id, req.raw);
		res.send({ ok: true });
	});

	app.get('/uploads/:id', async (req: FastifyRequest<{ Params: { id: string } }>, res) => {
		const id = req.params.id;
	...
	app.get('/unfurl', async (req: FastifyRequest<{ Querystring: { url: string } }>, res) => {
